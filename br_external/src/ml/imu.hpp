...
class IMU : protected i2cDevice
{
 public:
  ...
  int init();
  float getNewAngle();
  int writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, bool verify);
  bool isConnected();
  int writeConfigSet(const uint8_t *data, int size);
  int getFIFOCount();
  void calibrateGyro(int loops);
  void calibrateAccel(int loops);
  void PID(int readAddress, float kP, float kI, int loops);
  void enableDMP();
  void clearOffsets();
  void setAccelOffset(int16_t x_off, int16_t y_off, int16_t z_off);
  void setGyroOffset(int16_t x_off, int16_t y_off, int16_t z_off);
};
